...
class MLP(object):
    ...
    def createWideModel(self):
        ...
        wideModel.compile(optimizer=Adam(lr=self.init_lr, decay=self.init_lr / self.epochs),loss='categorical_crossentropy',metrics=['accuracy'])
        print("[INFO] training wide model on {0} dataset".format(self.dataset))
        wideH = wideModel.fit(self.X_train, self.Y_train, epochs=self.epochs, batch_size=self.batch_size,validation_data=(self.X_test, self.Y_test), verbose=0)
    ...
    def createDeepModel(self):
        ...
        deepModel.compile(optimizer=Adam(lr=self.init_lr, decay=self.init_lr / self.epochs),loss='categorical_crossentropy',metrics=['accuracy'])
        print("[INFO] training deep model on {0} dataset".format(self.dataset))
        deepH = deepModel.fit(self.X_train, self.Y_train, epochs=self.epochs, batch_size=self.batch_size,validation_data=(self.X_test, self.Y_test), verbose=0)
